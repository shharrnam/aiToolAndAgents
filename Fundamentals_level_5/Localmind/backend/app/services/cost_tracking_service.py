"""
Cost Tracking Service - Track and calculate API costs per project.

Educational Note: This service tracks Claude API usage costs by model.
...
class CostTrackingService:
    """
    Service for tracking API costs per project.

    Educational Note: Costs are tracked by model (sonnet/haiku) with
    separate input/output token counts. Total cost is calculated from
    these counts using current pricing.
    """
    ...
    def _get_model_key(self, model_string: str) -> str:
        """
    ...
    def _calculate_cost(self, model_key: str, input_tokens: int, output_tokens: int) -> float:
        ...
        Returns:
            Cost in USD
        """
        pricing = self.PRICING.get(model_key, self.PRICING["sonnet"])
    ...
    def _ensure_cost_tracking_structure(self, project_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def add_usage(
        ...
        Educational Note: This method is called after each Claude API call
        ...
        with self._lock:
            ...
            project_data = self._ensure_cost_tracking_structure(project_data)

            # Get model key and calculate cost
            model_key = self._get_model_key(model)
            call_cost = self._calculate_cost(model_key, input_tokens, output_tokens)
