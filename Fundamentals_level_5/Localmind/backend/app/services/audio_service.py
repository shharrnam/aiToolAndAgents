...
class AudioService:
    ...
    def _build_processed_content(
        ...
        """
        Build processed content with page markers.

        Educational Note: We use the same text splitting approach as text files
        (6500 chars per page with natural break points). This creates
        consistent chunking across all source types.
        ...
        Returns:
            Formatted content with AUDIO PAGE markers
        """
        from app.utils.text_utils import split_text_into_pages

        # Split transcript into pages
        pages = split_text_into_pages(transcript_text)
        total_pages = len(pages)

        # Build header
        content = f"# Transcribed from audio: {audio_name}\n"
        content += f"# Total pages: {total_pages}\n"
        content += f"# Model: {self.TRANSCRIPTION_MODEL}\n"

        # Add language info if detected
        if detected_language_code:
            lang_display = f"{detected_language_name} ({detected_language_code})" if detected_language_name else detected_language_code
            content += f"# Detected language: {lang_display}\n"

        content += f"# Processed at: {datetime.now().isoformat()}\n\n"

        # Add each page with marker
        for i, page_text in enumerate(pages, start=1):
            content += f"=== AUDIO PAGE {i} of {total_pages} ===\n\n"
